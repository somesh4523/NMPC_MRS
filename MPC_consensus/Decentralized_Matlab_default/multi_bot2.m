...
centroid_velocity = [0.2, 0]; % Velocity of the centroid in the positive x-direction
formation_achieved = false;
formationCenter = [0, 0]; % Will hold centroid once formation is achieved
...
% Generate raw formation vertices and center them around zero
rawPositions = generatePolygonVertices(numRobots, formationRadius);
relativeFormation = rawPositions - mean(rawPositions, 1);  % centered shape
...
for i = 1:numRobots
    ...
    nlobj{i}.Weights.OutputVariables = [5 5 0.1];
...
% Compute initial centroid of actual positions
centroid = mean(pos, 1);
...
for ct = 1:(Duration / Ts)
    ...
    if ~formation_achieved
        % Desired formation positions = current centroid + relativeFormation
        desiredPos = centroid + relativeFormation;
        v = Laplac * pos - Laplac * desiredPos;
        error = norm(v, 'fro');
        if error < 1
            formation_achieved = true;
            formationCenter = centroid;  % lock in the current centroid
            disp("Formation achieved. Starting centroid motion.")
        end
        centroid = mean(pos, 1);  % update current centroid
        % Consensus control step
        pos = pos + (kp * v - kc * (ones(numRobots, 1) * centroid)) * Ts;
        % Update yref for consensus phase
        for i = 1:numRobots
            yref{i} = [desiredPos(i, :), atan2(relativeFormation(i,2), relativeFormation(i,1))];
        end
    else
        % Move the locked-in formation center, then translate formation
        formationCenter = formationCenter + centroid_velocity * Ts;
        % Update yref to moving formation
        for i = 1:numRobots
            targetPos = formationCenter + relativeFormation(i, :);
            yref{i} = [targetPos, atan2(relativeFormation(i,2), relativeFormation(i,1))];
        end
    end
